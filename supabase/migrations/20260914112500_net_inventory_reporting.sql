-- Keep inventory reporting aligned with the signed ledger for sales, returns, cancellations and adjustments.
...
left join lateral (
  ...
  where si.product_id = p.id and si.owner_id = p.owner_id and sa.status = 'ACTIVE'
) s on true;
